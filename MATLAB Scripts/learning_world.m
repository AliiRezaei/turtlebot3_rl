...
% x axis collisions :
col_x_n = 3;
col_x = linspace(-1, 1, col_x_n);

% y axis collisions :
col_y_n = 3;
col_y = linspace(-1, 1, col_y_n);
...
collisions_row = col_x_n * col_y_n * col_theta_n;
collisions_col = 3;

% allocate memory for the collision_pairs matrix :
collision_pairs = zeros(collisions_row, collisions_col);

for i = 0:col_x_n - 1
    for j = 0:col_y_n - 1
        for k = 0:col_theta_n - 1
            index = i * (col_y_n * col_theta_n) + j * col_theta_n + k + 1;
            collision_pairs(index, 1) = col_x(i + 1);
            collision_pairs(index, 2) = col_y(j + 1);
            collision_pairs(index, 3) = col_theta(k + 1);
        end
    end
end
...
for e=1:n_episodes
    ...
    while true
        ...
        % check collisions :
        collision_status = find(ismember(collision_pairs, state, 'row'));
        if ~isempty(collision_status)
            break;
        else
            % epsilonilon greedy :
            if rand > epsilon
